...
A: `ES6` is not mandatory for `React` but is highly recommendable. The latest projects created on React rely a lot on ES6. React uses ES6, and we should be familiar with some of the new features like: Classes, Arrow Functions, Variables(let, const).
...
function App() {
  return (
    <div className="App">
      <Tool name="Vishal Kalita" tool="Tooljet"/> // name and tool are props
    </div>
  )
}
```
